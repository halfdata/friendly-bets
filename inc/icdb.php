...
class ICDB {
	...
	function __construct($_server, $_port, $_db, $_user, $_password, $_prefix) {
		...
		if (function_exists('mysqli_connect')) {
			...
			if (!$this->link) throw new Exception(sprintf(esc_html__('Could not connect: %s', 'fb'), mysqli_connect_error()));
			if (!mysqli_select_db($this->link, $this->db)) throw new Exception(sprintf(esc_html__('Can not use database: %s', 'fb'), mysqli_error($this->link)));
			if (!mysqli_query($this->link, 'SET NAMES utf8')) throw new Exception(sprintf(esc_html__('Invalid query: %s', 'fb'), mysqli_error($this->link)));
		} else {
			$this->link = mysql_connect($host, $this->user, $this->password);
			if (!$this->link) throw new Exception(sprintf(esc_html__('Could not connect: %s', 'fb'), mysql_error()));
			if (!mysql_select_db($this->db, $this->link)) throw new Exception(sprintf(esc_html__('Can not use database: %s', 'fb'), mysql_error($this->link)));
			if (!mysql_query('SET NAMES utf8', $this->link)) throw new Exception(sprintf(esc_html__('Invalid query: %s', 'fb'), mysql_error($this->link)));
		}
	}
	...
	function query($_sql) {
		if ($this->use_mysqli) {
			...
			if (!$result) throw new Exception(sprintf(esc_html__('Invalid query: %s', 'fb'), mysqli_error($this->link)));
		} else {
			$result = mysql_query($_sql, $this->link);
			if (!$result) throw new Exception(sprintf(esc_html__('Invalid query: %s', 'fb'), mysql_error($this->link)));
		}
